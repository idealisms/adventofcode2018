...
UNITS_ORIG = [u[:] for u in units]
for boost in range(1, 10000):
  units = [u[:] for u in UNITS_ORIG]
  for unit in units:
    if unit[TEAM] == 'immune':
      unit[POWER] += boost
  # 82 and 83 result in a stalemate, so give up after 20,000 turns.
  for t in range(20000):
    print '{} boost, turn {}'.format(boost, t)
    # target selection
    ...
    if immune_units == 0:
      ...
      print 'Immune wins (with a boost of {}): {} units left.'.format(boost, immune_units)
      sys.exit(0)
